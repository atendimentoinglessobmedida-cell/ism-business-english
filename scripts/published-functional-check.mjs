...
assert(html.includes('core-shell.css?v=1')&&html.includes('learner-features.js?v=1'),'Core modular assets missing');assert(features.includes('startSmartReview')&&features.includes('openBusinessMode'),'Learner feature module incomplete');assert(coreCss.includes('Core shell base styles'),'Core shell stylesheet missing');assert(storage.includes("mode:storageAvailable?'persistent':'session'"),'Session-safe storage fallback missing');assert(/id="home" class="panel on"/.test(html),'Home panel missing');
...
assert(sw.includes("const CACHE='ism-business-v47'"),'Expected current PWA cache version');
